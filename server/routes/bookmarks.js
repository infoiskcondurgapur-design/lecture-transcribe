...
function readBookmarks(req) {
  const raw = req.cookies?.bookmarks;
  if (!raw) return [];
...
function setBookmarksCookie(res, ids) {
  const value = signIds(ids);
  res.cookie('bookmarks', value, {
    httpOnly: true,
    sameSite: 'lax',
...
export async function getBookmarksIds(req) {
  return readBookmarks(req);
}
